...
public class ArrayList<E> implements List<E> {
    ...
    @Override
    public void insertarOrdenado(E elem, Comparator<E> cmp) {
        //
    }
    ...
    /**
     * Éste método realmente no debe ser privado...
     *
     * @param elem elemento a buscar
     * @return índice del elemento a buscar
     */
    private int indexOf(final E elem) {
        return IntStream.range(0, size)
                        .filter(i -> this.get(i).equals(elem))
                        .findFirst()
                        .orElse(-1);
    }

    @Override
    public void removerElementos(final List<Integer> xs) {
        /* copiar colección para mantener información de índices */
        final List<E> ys = new ArrayList<>();
        this.forEach(ys::addLast);
